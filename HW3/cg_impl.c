...
void conj_grad(int colidx[],
    ...
    #pragma omp parallel
    {
        ...
        int L = firstcol + id * (N / nthrds);
        int R = (id == nthrds - 1) ? (lastcol + 1) : (L + (N / nthrds));
        for (j = L; j < R; j++)
        {
            thread_sum[id] += r[j] * r[j];
        }
    }
...
void iterate(double *zeta, int *it) {
    ...
    double norm_temp1s[NUM_THREADS];
    double norm_temp2s[NUM_THREADS];
    for(int i = 0; i < NUM_THREADS; i++) {
        norm_temp1s[i] = 0.0;
        norm_temp2s[i] = 0.0;
    }
    ...
    #pragma omp parallel
    {
        ...
        for (j = l; j < r; j++)
        {
            norm_temp1s[id] += x[j] * z[j];
            norm_temp2s[id] += z[j] * z[j];
        }
    }

    
    for(int i = 0; i < NUM_THREADS; i++) {
        norm_temp1 += norm_temp1s[i];
        norm_temp2 += norm_temp2s[i];
    }
#else
    for (j = 0; j < lastcol - firstcol + 1; j++)
    {
