...
__global__ void mandelKernel(int *img, int maxIterations, float stepX, float stepY, float lowerX, float lowerY, int resX, int resY, size_t pitch, int thread_X, int thread_Y) {
    ...
    for(int i = 0; i < thread_X; ++i) {
        for(int j = 0; j < thread_Y; ++j) {
            int thisX = _X * thread_X + i;
            int thisY = _Y * thread_Y + j;
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    const int thread_X = 2;
    const int thread_Y = 2;
    ...
    mandelKernel<<<numBlocks, numThreads>>>(cudaMem, maxIterations, stepX, stepY, lowerX, lowerY, resX, resY, pitch, thread_X, thread_Y);
